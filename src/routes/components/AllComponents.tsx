    ...
    <GoabTable width="100%" onSort={sortData}>
      <thead>
        <tr>
          ...
          <th>
            <GoabTableSortHeader name="groups">Category</GoabTableSortHeader>
          </th>
          ...
          <GoabGrid minChildWidth="15rem" gap="xl">
            {filteredCards.map(card => (
              <ComponentCard
                ...
                status={card.status}
                githubLink={`https://github.com/GovAlta/ui-components/issues?q=is%3Aissue+is%3Aopen+label%3A${encodeURIComponent(
                  getLabelQuery(card.name)
                )}`}
