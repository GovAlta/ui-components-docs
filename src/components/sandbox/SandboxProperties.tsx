...
export function SandboxProperties({ properties = [], onChange }: Props) {
  ...
  function onCheckboxChange(
    ...
    const prop = properties.find(
      (p) => p.type === "boolean" && p.name === name
    );
    if (!prop || prop.type !== "boolean") return;
    prop.value = checked;
    prop.checkboxValue = _value && _value !== "checked" ? _value : ""; // GoACheckbox returns "checked" when true
    onChange([...properties]);
  }
      ...
      case "boolean":
        return (
          <GoACheckbox
            ...
            name={p.name}
            checked={p.value}
            value={p.checkboxValue ? p.checkboxValue : ""}
  ...
  return (
    <>
      {["list", "boolean", "string", "number"].map((t) => {
        const propertiesOfTypeT = properties.filter(
          (p) => p.type === t && !p.hidden
        );
